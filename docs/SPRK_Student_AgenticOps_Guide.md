...
| --- | --- | --- |
| Instruction source | Private vault hierarchy with L1/L2/L3 instructions | Public repo uses `AGENTS.md`, this guide, and mission docs; no private vault content is copied here |
...
Codespaces-compatible environments run setup from `.devcontainer/devcontainer.json`. GitHub Actions runs setup and mission validation from `.github/workflows/mission-baseline.yml`.
...
1. Keep `AGENTS.md` and this guide as the public MERIT-to-SPRK bridge.
2. Add lightweight checks to `ops/` only when they help students avoid common mistakes.
